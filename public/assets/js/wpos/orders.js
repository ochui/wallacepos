function WPOSOrders(){
    ...
    this.processOrder = function(orderdata, olddata){
        var device = POS.getConfigTable().deviceconfig;
                ...
                if (data.hasOwnProperty('orderdata')){
                        ...
                        if (olddata.orderdata.hasOwnProperty(i)){
                            ...
                            if (data.orderdata[i].hasOwnProperty('moddt')){
                                ...
                                if (!olddata.orderdata[i].hasOwnProperty('moddt') || data.orderdata[i].moddt>olddata.orderdata[i].moddt) {
                                    ...
                                    if (display){
                                        POS.orders.removeOrder(data.ref, i);
                                        insertOrder(data, i);
                                    }
                                    if (print)
                                        POS.print.printOrderTicket(print, data, i, "ORDER UPDATED");
                                    modcount++;
                                }
                            }
                        } else {
                            ...
                            if (display)
                                insertOrder(data, i);
                            if (print)
                                POS.print.printOrderTicket(print, data, i, null);
                            modcount++;
                        }
                    }
                } else {
                    ...
                    if (olddata.hasOwnProperty('orderdata'))
                        for (var r in olddata.orderdata){
                            if (display)
                                POS.orders.removeOrder(olddata.ref, r);
                            if (print)
                                POS.print.printOrderTicket(print, olddata, r, "ORDER CANCELLED");
                            modcount++;
                        }
                }
            } else {
                ...
                if (data.hasOwnProperty('orderdata'))
                    for (var o in data.orderdata) {
                        if (display)
                            insertOrder(data, o);
                        if (print)
                            POS.print.printOrderTicket(print, data,o, null);
                        modcount++;
                    }
            }
        } else {
            ...
            if (olddata){
                if (olddata.hasOwnProperty('orderdata'))
                    for (var d in olddata.orderdata){
                        if (display)
                            POS.orders.removeOrder(olddata.ref, d);
                        if (print)
                            POS.print.printOrderTicket(print, olddata, d, "ORDER CANCELLED");
                        modcount++;
                    }
            }
        }
