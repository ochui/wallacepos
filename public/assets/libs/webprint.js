...
var WebPrint = function (init, opt) {
    ...
    this.printSerial = function (data, port) {
        if (isAndroid){
            if (typeof POS!== 'undefined' && POS.notifications) {
                POS.notifications.error("Serial port printing is not available in Android.", "Platform Limitation", {delay: 0});
            } else {
    ...
    this.printHtml = function (data, printer) {
        if (isAndroid){
            if (typeof POS!== 'undefined' && POS.notifications) {
                POS.notifications.error("HTML printing is not available in Android.", "Platform Limitation", {delay: 0});
            } else {
        ...
        switch (event.data.a) {
            ...
            case "response":
                ...
                if (response.hasOwnProperty('ports')) {
                    ...
                    if (typeof POS!== 'undefined' && POS.notifications) {
                        POS.notifications.error(response.error, "WebPrint Error", {delay: 0});
                    } else {
    ...
    function dispatchWebPrint() {
        if (typeof POS!== 'undefined' && POS.util && POS.util.confirm) {
            POS.util.confirm("Cannot communicate with the printing app.\nWould you like to open/install the printing app?", function() {
                if (isAndroid){
                    deployAndroid();
                    return;
                }
