...
/**
 *
 * SuppliersModel extends the DbConfig PDO class to interact with the config DB table
 *
 */
...
class SuppliersModel extends DbConfig
{
    ...
    {
        $sql = 'SELECT s.id, s.name, s.dt, COUNT(i.id) as numitems FROM stored_suppliers as s LEFT OUTER JOIN stored_items as i ON s.id=i.supplierid';
        $placeholders = [];
        if ($Id !== null) {
