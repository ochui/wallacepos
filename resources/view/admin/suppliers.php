...
<script type="text/javascript">
    ...
    $(function() {
        ...
        datatable = $('#supplierstable').dataTable({
            ...
            "aoColumns": [
                ...
                { "mData":"name" },
                { "mData": "numitems"},
                { "mData":function(data,type,val){return '<div class="action-buttons"><a class="green" onclick="openeditsupdialog('+data.id+');"><i class="icon-pencil bigger-130"></i></a><a class="red" onclick="removeSupplier('+data.id+')"><i class="icon-trash bigger-130"></i></a></div>'; }, "bSortable": false, sClass: "noexport" }
            ],
            "columns": [
                {},
                {type: "numeric"},
    ...
    function removeSupplier(id){
        ...
        POS.util.confirm("Are you sure you want to delete this supplier?", function() {
            // show loader
            POS.util.showLoader();
            var result = POS.sendJsonData("suppliers/delete", '{"id":'+id+'}');
            if (result !== false){
                delete suppliers[id];
                reloadTable();
            }
    ...
    function removeSelectedSuppliers(){
        ...
        POS.util.confirm("Are you sure you want to delete "+ids.length+" selected items?", function() {
            // show loader
            POS.util.showLoader();
            var result = POS.sendJsonData("suppliers/delete", '{"id":"'+ids.join(",")+'"'}');
            if (result !== false){
